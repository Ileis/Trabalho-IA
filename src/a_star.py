...
def a_star(g: Graph, start: Position, end: Position,fun_cost: Callable[[Node, Position], int], heuristic: Callable[[Node, Position], int]) -> Path:
    ...
    while heap.empty() is False:
        ...
        for move in current.moves:
            ...
            h_cost: int = heuristic(current, neighbor_position) + current.h + fun_cost(current, neighbor_position)
            if not is_visited(visited, neighbor_position):
                ...
                neighbor: Node = heap.harr[heap.find(neighbor_node)]
                if neighbor is not None and neighbor.h > h_cost:
                    neighbor.h = h_cost
    return []